...
export function calculateCampaignEconomics(input: CampaignEconomicsInput): CampaignEconomics {
  ...
  const seen = new Set<string>(); const rows = records.flatMap((attribution) => {
  ...
  const revenue = rows.length ? addMoney(...rows.map(({ order, share }) => allocated(subtractMoney(addMoney(order.merchandiseGross, order.shippingCharged), addMoney(order.discounts, order.refundedAmount)), share))) : undefined;
  const trueCost = missingInputs.some((message) => message.startsWith("Complete True Cost")) ? undefined : rows.length ? addMoney(...rows.map(({ trueCost: result, share }) => allocated(result!.trueCost, share))) : undefined;
  const trueProfit = missingInputs.some((message) => message.startsWith("Complete True Cost")) ? undefined : rows.length ? addMoney(...rows.map(({ trueCost: result, share }) => allocated(result!.trueProfit!, share))) : undefined;
  const spend = input.campaign.spend; const acquiredCustomers = new Set(rows.map(({ attribution }) => attribution.customerId).filter(Boolean)).size;
