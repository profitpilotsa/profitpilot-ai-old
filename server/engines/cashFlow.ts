...
export function calculateCashFlow(input: CashFlowInput): CashFlowProjection {
  ...
  const obligationsStatus = input.obligationsKnown ? "provided" : "none_known"; const obligations = input.knownObligations ?? zeroMoney;
  const projectedCash = input.openingCash === undefined || missingInputs.some((item) => item.startsWith("Currency")) ? undefined : subtractMoney(addMoney(input.openingCash, inflows), addMoney(outflows, obligations));
