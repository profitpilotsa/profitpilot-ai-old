...
export function adaptOrderDisplay(order: Order, items: OrderItem[], customerReference?: string): OrderDisplay {
  const reversalIncomplete = order.status === "refunded" || order.status === "partially_refunded" || order.status === "returned";
  const cancelled = order.status === "cancelled";
  const profitabilityStatus: OperationalStatus = cancelled ? "no_data" : reversalIncomplete ? "incomplete" : "estimated";
  return { id: order.externalId ?? order.id, source: order.source === "demo" ? "Demo" : order.source, orderedAt: order.orderedAt, status: order.status, paymentStatus: order.status === "paid" ? "Paid" : order.status.replace(/_/g, " "), customerReference, items: items.map((item) => ({ title: item.title, quantity: item.quantity, revenue: presentMoney(item.unitGross) })), revenue: presentMoney(order.merchandiseGross), discounts: presentMoney(order.discounts), trueCostStatus: profitabilityStatus, trueProfitStatus: profitabilityStatus };
}
export function demoOrderDisplay(): OrderDisplay { return adaptOrderDisplay(demoOrder, demoOrderItems, "Demo order"); }
